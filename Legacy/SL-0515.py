...
def calc_tm_boltzmann(T, F):
    ...
    p0 = [F[0]-F.min(), 0.01, F.min(),
          F.max()-F.min(), 0.005, F.min(),
          np.median(T), 2.0]
    try:
        popt2, pcov2 = curve_fit(boltzmann_exp, T, F, p0=p0, maxfev=20000)
    except RuntimeError:
    ...
    try:
        popt1,_ = curve_fit(one_state,T,F,p0=[F[0]-F.min(),0.01,F.min()],maxfev=10000)
        rss1 = np.sum((F-one_state(T,*popt1))**2)
    except RuntimeError:
        rss1 = rss2
    n=len(T); aic1=n*np.log(rss1/n)+2*3; aic2=n*np.log(rss2/n)+2*8
    delta_aic = aic1 - aic2
